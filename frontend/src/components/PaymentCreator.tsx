...
export default function PaymentCreator({ onPaymentCreated }: { onPaymentCreated?: () => void }) {
  ...
  const onSubmit = async (e: React.FormEvent) => {
    ...
    // Call the callback when payment is successfully created
    if ('data' in result && result.data && onPaymentCreated) {
      onPaymentCreated();
    }
  };
  ...
  return (
    <div className="space-y-6">
      <div className="bg-white rounded-lg shadow p-6">
        <h2 className="text-2xl font-bold text-gray-800 mb-6">Crea un nuovo pagamento</h2>
        ...
        <form className="space-y-6" onSubmit={onSubmit}>
          ...
          <div className="flex gap-3">
            <button
              className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 disabled:cursor-not-allowed text-white font-semibold py-3 px-4 rounded-md transition duration-200 flex items-center justify-center gap-2"
              type="submit"
              disabled={isLoading}
            >
            ...
            {isSuccess && (
              <button
                type="button"
                onClick={resetForm}
                className="px-6 bg-gray-600 hover:bg-gray-700 text-white font-semibold py-3 rounded-md transition duration-200"
              >
                Nuovo
              </button>
            )}
